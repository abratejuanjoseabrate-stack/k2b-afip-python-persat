...
from .schemas import FacturaCreate, FacturaCCreate, FacturaResponse, PuntoVenta, IVACondition
...
class WSFEv1Service:
    ...
    def _conectar(self):
        ...
        """
        logger.info(f"Conectando WSFEv1Service en ambiente: {self.env}")
        ...
        logger.debug(f"CUIT: {settings.CUIT}")
        logger.debug(f"Cert: {cert_path}")
        ...
        if not ok:
            ...
            if excepcion:
                error_msg += f": {excepcion}"
            raise AFIPConnectionError(
                message=error_msg,
                service="wsfe",
                wsdl=wsdl,
                soap_request=getattr(self.wsfev1, 'XmlRequest', None),
                soap_response=getattr(self.wsfev1, 'XmlResponse', None),
            )
        ...
        logger.info("Conexión WSFEv1 establecida correctamente")
