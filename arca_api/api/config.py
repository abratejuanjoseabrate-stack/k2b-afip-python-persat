...
class Settings(BaseSettings):
    ...
    def base_dir(self) -> Path:
        ...
        if self._base_dir is None:
            # Obtener desde el archivo actual (api/config.py)
    ...
    API_RELOAD_DIRS_STR: str = Field(default="api", alias="API_RELOAD_DIRS")
    ...
    def API_RELOAD_DIRS(self) -> list[str]:
        ...
        if not self.API_RELOAD_DIRS_STR:
            return ["api"]
        return [p.strip() for p in self.API_RELOAD_DIRS_STR.split(",") if p.strip()] or ["api"]
