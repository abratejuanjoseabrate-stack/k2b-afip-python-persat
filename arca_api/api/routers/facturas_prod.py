"""
Router de facturas - Endpoints REST para WSFEv1 (PRODUCCIÓN)
"""
...
router = APIRouter(prefix="/api/v1/facturas", tags=["facturas produccion"])


@router.post("", response_model=FacturaResponse, status_code=201)
def crear_factura_prod(factura: FacturaCreate):
    """
    Crea cualquier tipo de comprobante electrónico y obtiene el CAE (producción)
...
def crear_factura_c_prod(factura: FacturaCCreate):
...
def consultar_comprobante_prod(
    ...
    """
    Consulta un comprobante que **ya existe** en AFIP (producción).
...
def obtener_puntos_venta_prod():
    """
    Obtiene la lista de puntos de venta registrados (producción)
    ...
    try:
        ...
        print("[PUNTOS_VENTA_PROD]", puntos_venta)
...
def obtener_condiciones_iva_receptor_prod(clase_cmp: str = Query("A", description="Clase de comprobante para filtrar las condiciones (A, B o M)")):
    """
    Obtiene la lista de condiciones de IVA de receptores válidas (producción).
...
def obtener_tipos_comprobante_prod():
    """
    Obtiene la lista de tipos de comprobante disponibles (producción)
...
def obtener_tipos_documento_prod():
    """
    Obtiene la lista de tipos de documento disponibles (producción)
...
def obtener_tipos_iva_prod():
    """
    Obtiene la lista de tipos de IVA (alícuotas) disponibles (producción)
...
def obtener_tipos_concepto_prod():
    """
    Obtiene la lista de tipos de concepto disponibles (producción)
